import { Blockchain_Saturnalia_Workshop_backend } from "../../declarations/todo_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await Blockchain_Saturnalia_Workshop_backend.addTodo(obj.textarea);
    showUserOnScreen({ id: getId, textarea: obj.textarea });
    textarea.value = ""; 
  } catch (error) {
    console.error("Error adding todo_Workshop:", error);
  }
});

window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await Blockchain_Saturnalia_Workshop_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await Blockchain_Saturnalia_Workshop_backend.deleteTodo(userId);
